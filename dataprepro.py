...
def _process_article(article, config):
    ...
    label = []
    for i in range(10):
        label.append(1) if i in gold_para else label.append(0)
    ...
    example = {'context_tokens': context_tokens,'context_chars': context_chars,
               ...
               'y2s': [best_indices[1]], 'id': article['_id'], 'start_end_facts': start_end_facts, "gold_para": label}
    eval_example = {'context': text_context, 'spans': flat_offsets, 'answer': [answer], 'id': article['_id'], "gold_para": label,
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict):
    ...
    for example in tqdm(examples):
        ...
        datapoints.append({'context_idxs': torch.from_numpy(context_idxs),
            ...
            'y2': y2,
            'id': example['id'],
            'label': torch.from_numpy(np.array(example["gold_para"])),
