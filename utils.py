...
def para_sent_mapping(para_spans, sent_spans):
    if len(para_spans) == 0:
        return {}, {}

    para_sent_dict = {i: [] for i in range(len(para_spans))}
    sent_para_dict = {i: -1 for i in range(len(sent_spans))}
    pi = 0
    for i in range(len(sent_spans)):
        ...
        if es >= ps and ee <= pe:
            para_sent_dict[pi].append(i)
            sent_para_dict[i] = pi

    return para_sent_dict, sent_para_dict
...
# def create_hierarchical_graph(case, max_query_entity_num, max_entity_num, max_sent_num, max_para_num, para_limit, graph_type, self_loop, single_entity, relational=False, debug=False):
#     # print('\n\n' + case.qas_id)
...
#
#     if self_loop:
#         adj = np.eye(max_query_entity_num + max_entity_num + max_sent_num + max_para_num, dtype=np.float32)
#     else:
#         adj = np.zeros((max_query_entity_num + max_entity_num + max_sent_num + max_para_num, max_query_entity_num + max_entity_num + max_sent_num + max_para_num), dtype=np.float32)
#
#     if graph_type.startswith('sent') and not single_entity:
#         query_entity = case.query_entities[:max_query_entity_num]
#         entities = case.entity_spans[:max_entity_num]    #ent_start_position, ent_end_position, entity_text, entity_type
#         sent_spans = case.sent_spans[:max_sent_num]
#         para_spans = case.para_spans[:max_para_num]
#         #question entities
#
#         # delect entities that out bound
#         for i in range(len(entities)-1, -1, -1):
#             es, ed, _ = entities[i]
#             if es >= para_limit:
#                 del entities[i]
#
#         #delect para_spans that out bound
#         for i in range(len(sent_spans)-1, -1, -1):
#             ss, sd = sent_spans[i]
#             if ss >= para_limit:
#                 del sent_spans[i]
#         # delect sent_spans that out bound
#         for i in range(len(para_spans)-1, -1, -1):
#             ps, pd, _ = para_spans[i]
#             if ps >= para_limit:
#                 del para_spans[i]
#
#         #entities to sent mapping
#         sent_ent_dict, ent_sent_dict = sent_mapping(sent_spans, entities)
#         #entities to para mapping
#         para_ent_dict, ent_para_dict = para_mapping(para_spans, entities)
#         #sent to para mapping
#         para_sent_dict, sent_para_dict = para_sent_mapping(para_spans, sent_spans)
#         #add adj if node in sents
#         for i, v in sent_ent_dict.items():
#             for j in v:
#                 adj[i+max_entity_num][j] = adj[j][i+max_entity_num] = 1
#         #add adj if sents in para
#         for i, v in para_sent_dict.items():
#             for j in v:
#                 adj[i+max_entity_num+max_sent_num][j] = adj[j][i+max_entity_num+max_sent_num] = 1
#         #add adj if entity is same
#         same_pairs = []
...
#         #add adj if q nodes appear in sent
#         # same_sents = []
#         # for i in range(len(entities)-1):
#         #     _, _, a, _ = entities[i]
#         #     for j in range(i+1, len(entities)):
...
#         #         same_sent = ent_sent_dict[i] == ent_sent_dict[j]
#         #         same_para = ent_para_dict[i] == ent_para_dict[j]
#         #         if same_sent and same_para:
#         #             same_sents.append([i, j])
#
#         #add adj if q nodes appear in sent and q nodes with sent
...
#         entity_mapping = np.zeros((max_entity_num+max_sent_num+max_para_num, para_limit), dtype=np.float32)
#         for i, ent in enumerate(entities):
#             es, ed, _ = ent
#             entity_mapping[i, es:ed+1] = 1
#         for i, sent in enumerate(sent_spans):
#             ss, sd = sent
#             entity_mapping[i+max_entity_num, ss:sd+1] = 1
#         for i, para in enumerate(para_spans):
#             ps, pd, _ = para
#             entity_mapping[i+max_sent_num+max_entity_num, ps:pd+1] = 1
#
...
def create_hierarchical_graph(case, max_query_entity_num, max_entity_num, max_sent_num, max_para_num, para_limit, graph_type, self_loop, single_entity, relational=False, debug=False):
    ...
    if self_loop:
        adj = np.eye(max_entity_num + max_sent_num + max_para_num, dtype=np.float32)
    else:
        adj = np.zeros((max_entity_num + max_sent_num + max_para_num,  max_entity_num + max_sent_num + max_para_num), dtype=np.float32)
    if graph_type.startswith('sent') and not single_entity:
        entities = case.entity_spans[:max_entity_num]    #ent_start_position, ent_end_position, entity_text, entity_type
        sent_spans = case.sent_spans[:max_sent_num]
        para_spans = case.para_spans[:max_para_num]
