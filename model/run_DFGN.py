...
from model.GNN import BertForQuestionAnswering, GraphFusionNet, DFGN
...
def main():
    ...
    model = DFGN.from_pretrained(args.bert_model, graph_config=args)
    model.to(device)
    model.train()
    ...
    if args.do_train:
        ...
        train_data = DataIteratorPack(train_features, train_examples_dict, train_graph, args.train_batch_size, device, sent_limit=25, entity_limit=80,
        ...
        eval_data = DataIteratorPack(eval_features, eval_examples_dict, eval_graph, args.predict_batch_size, device, sent_limit=25, entity_limit=80,
                                      n_layers=args.n_layers, sequential=False)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(train_data):
                ...
                start, end, sp, Type, softmask, ent, yp1, yp2 = model(input_ids, segment_ids, input_mask, batch=batch, return_yp=True, is_train=True)
            ...
            if args.do_predict:
                ...
                #         start_logits = start[i].detach().cpu().tolist()
                #         end_logits = end[i].detach().cpu().tolist()
                #         # eval_feature = eval_features[example_index.item()]
                #         unique_id = batch['unique_id'][i]
                #
                #         types = Type[i].detach().cpu().tolist()
                ...
                #     print("Test Loss{}: {}".format(i, l / len(eval_data)))
