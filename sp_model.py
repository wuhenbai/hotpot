...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.linear_para = nn.Linear(config.hidden * 2, 20)
        self.cache_S = 0
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        predict_para = self.linear_para(output_type)
        predict_para = predict_para.view(-1, 2, 10)

        if not return_yp: return logit1, logit2, predict_type, predict_support, predict_para
        ...
        return logit1, logit2, predict_type, predict_support, yp1, yp2, predict_para
