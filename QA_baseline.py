...
def get_train_feature(args, is_train, tokenizer):
    ...
    if not os.path.exists(examples_file):
        train_examples = read_hotpot_examples(data_file, select_file,  is_train)
...
def main():
    ...
    if args.do_train:
        ...
        train_data = DataIteratorPack(train_features, train_examples, args.train_batch_size, device, sent_limit=25,
                                      sequential=False)

        # load dev data
        eval_examples, eval_features = get_train_feature(args, not args.do_train, tokenizer)

        eval_data = DataIteratorPack(eval_features, eval_examples, args.predict_batch_size, device, sent_limit=25,
                                      sequential=False)
