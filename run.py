...
def train(config):
    ...
    create_exp_dir(config.save, scripts_to_save=['run.py', 'NER_model.py', 'util.py', 'sp_model.py'])
    ...
    for epoch in range(10000):
        for data in build_train_iterator():
            ...
            logit1, logit2, predict_type, predict_support, pred_para = model(context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False)
            loss_1 = (nll_sum(predict_type, q_type) + nll_sum(logit1, y1) + nll_sum(logit2, y2)) / context_idxs.size(0)
            loss_2 = nll_average(predict_support.view(-1, 2), is_support.view(-1))
            loss_3 = nll_average(pred_para, label)

            loss = loss_1 + config.sp_lambda * loss_2 +loss_3
...
def evaluate_batch(data_source, model, max_batches, eval_file, config):
    ...
    for step, data in enumerate(iter):
        ...
        logit1, logit2, predict_type, predict_support, yp1, yp2, pred_para = model(context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=True)
        loss = (nll_sum(predict_type, q_type) + nll_sum(logit1, y1) + nll_sum(logit2, y2)) / context_idxs.size(0) + config.sp_lambda * nll_average(predict_support.view(-1, 2), is_support.view(-1)) + nll_average(pred_para, label)
        loss_3 = nll_average(pred_para, label)
...
def predict(data_source, model, eval_file, config, prediction_file):
    ...
    for step, data in enumerate(tqdm(data_source)):
        ...
        answer_dict.update(answer_dict_)
        #
        pred_para = torch.argmax(pred_para, dim=1)
        ...
        for i in range(predict_support_np.shape[0]):
            ...
            cur_id = data['ids'][i]
            #
            gold_para.update({cur_id: pred_para[i]})
